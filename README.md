...
| 2 | "Branching consequence" template | A choice that changes which ending the player gets. "Two Ways Out" is approved (may be morally grey) but not yet built. The official Cryptographer Hunt (a phone social-engineering scene with a fail route on the wrong choice) is the strongest argument for it — see [`docs/plans/r127-official-quest-comparison.md`](docs/plans/r127-official-quest-comparison.md). The shape now ships inside The Long Game (r136, act III: a typed verdict with two endings) and inside Dead Air (r232, a social-engineering call whose wrong word ends the quest on a cheaper ending); whether a standalone template still adds anything is Zeis's call. |
...
Counted from the code at build `2026-09-26.r232`: **1,861 tests** across 91
