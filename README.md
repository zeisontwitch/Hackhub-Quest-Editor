...
| 11 | **Website builder polish** | The two remaining approved r132 proposals: preview navigation (a mini address bar so the preview follows in-site links — walk the site like a player) and multi-file import (the AI flow returns one .html per page; import in one go, creating pages from filenames, `news.html` → `/news`). |
...
| Round | Item |
|---|---|
| r133 | **Linking without touching HTML** ([plan](docs/plans/r133-page-linking.md)): Zeis proposed an After Effects pick-whip (drag a wire from the sidebar page onto the text that should link to it); the verdict — right instinct, wrong physics for an iframe — shipped as **click-click**: the 🔗 toolbar button is now a popover listing the site's pages (link the selection, or insert the path bare), and the 🎯 beside each page arms *point-to-link* — next click inside the page becomes the link, existing links retarget. Pure DOM helpers in `pageDoc.ts`; drag version possible later if the 🎯 misses the noodle. |
