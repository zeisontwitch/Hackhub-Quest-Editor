...
| # | Item | Notes |
|---|---|---|
| 1 | **Twotter comes back — plan for review** | ([plan](docs/plans/r185-twotter-return.md)) The probe's evidence says the feature can return on the API path. The plan: mod-level **Twotter accounts** (handle, display name, bio, avatar/banner, verified, follower counts, and a per-account "remove when the quest ends" toggle), a rebuilt **Post tweet** node (post once per playthrough, removed on quest end, timestamps computed from the in-game clock instead of the old `postedAgo` string that produced the moment.js warning), a strict **fence set** against the r31 shape (never emit the declarative `TwotterAccounts`/`Tweets` fields; assert a blank bio still creates `bio: ""`), a **migration** that maps pre-r31 drafts instead of deleting their tweets, an in-game QA round **T-08…T-13** and a `qe24 twotter audit` command, and the **whimsy** pass (live tweet preview, account cards, the canvas card reading like a tweet). Four open questions at the end want Zeis's call before it is built. |
