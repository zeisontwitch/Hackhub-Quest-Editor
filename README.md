...
| # | Item | Notes |
|---|---|---|
| 1 | **Cheap wins: pack extras and localization — two rows open (G, H)** | ([plan](docs/plans/r199-cheap-wins-plan.md)) **Stage A closed**, **Stage B built**. In game: the widget, both right-click entries and the German quest title are green; **`UI.notify` and `UI.toast` both draw** (so the editor's default Notify variant was never silent). The click on a pack's menu item reaches the pack and is then **refused for lack of a mod identity** — `[ContentSDK] Mod "null" tried to use UI.toast without "ui" permission`, while the *same export in the same session* showed a quest-context notification fine and its manifest lists `ui`. Filed as **Q14** in `docs/03-questions-for-the-developers.md`; the runtime explains the refusal in the log when it sees it. Row **H** (`qe24 clickprobe on|report|off`) asks whether a click still has ungated channels and whether a deferred `Scheduler` job gets the identity back — that answer decides how the editor's four click actions can work. Install export **1.0.30** + harness **1.0.23**. |
