...
| Accounts outlive an uninstall | A mod's Twotter accounts and posts are **not** removed when the mod is uninstalled — the SDK says so, and the game drops the mod's quests but keeps the accounts a mod created through `createUser`. Our runtime cleans up in `OnModPackageUnloaded`, the SDK's documented moment for this — but **measured 2026-09-19: that hook is not called when the player simply quits to desktop**, and it can never be called for a mod deleted from disk, so the advice is unreachable in practice. Completing or abandoning the story removes the accounts (verified in game, T-11b/T-12b). Question filed: [`docs/03`](docs/03-questions-for-the-developers.md) §11. |
...
Counted from the code at build `2026-09-18.r188`: **1,714 tests** across 85
