...
| # | Item | Notes |
|---|---|---|
| 1 | **Twotter comes back — plan, revision 2** | ([plan](docs/plans/r185-twotter-return.md)) Zeis's answers are folded in. Mod-level **Twotter accounts** (handle, display name, bio, avatar/banner, verified, counts, per-account "remove when the quest ends" toggle), removed only if the quest created them and no other live quest still declares them. The node — named **"Twotter"** — posts a **list of tweets**, each with its own time (when the story arrives, or earlier by amount + unit), so a profile can be found carrying a month of history like @alinamack's in the Journalist's Sister line. A **migration** maps pre-r31 drafts instead of deleting their tweets; a strict **fence set** keeps the declarative `TwotterAccounts`/`Tweets` fields out of every build; QA rows **P-01** then T-08…T-15 plus `qe24 twotter audit`. The **whimsy**: the canvas card stays a normal card, the click-to-edit **mock profile** lives in the Twotter panel, and the node previews the tweet timeline. **Both probes are green** — the platform keeps a `sendedAt` we send (all three spellings read "a month ago"), and a profile sorts by time with the **newest at the top**, so the preview mirrors the game. Result tables: [`reference/sdk-0.24-qa/P-01-BACKDATE.md`](reference/sdk-0.24-qa/P-01-BACKDATE.md). **Stage 1 is built and field-tested** (2026-09-18): T-08…T-10, T-12, T-13 and T-14 passed in game, and the run found three runtime bugs — a quest-start hook that never ran, "live" counting quests that had never started, and a finished quest that could never post again — all fixed in r186 with falsified fences. Tweet pictures do not survive the posting API (**SDK 0.24's record has no picture field**), so the editor now says so instead of promising one. The 2026-09-19 pass closed most of them — **T-11b (completion) green, T-12b green** — and the picture question is settled: **SDK 0.24's `TwotterTweet` has no picture field and the picture never reaches the feed**, so the editor's upload control is hidden (restoring it is one line), the runtime keeps sending the key, a **feature request** is filed as question 10 in [`docs/03-questions-for-the-developers.md`](docs/03-questions-for-the-developers.md), and the QA fixtures now use unmistakable colours (**#AA28FF** banner, amber avatar) for T-08b. **Stage 2 (the whimsy) is built in r188:** the Twotter panel is a click-to-edit mock profile — banner, avatar, name, handle, bio, blue check all edit in place, both pictures open their picker, and the panel lists what the account has posted across every quest — while the node inspector shows the same profile read-only over its timeline, **newest first** the way the game shows it, with one shimmering "posting" row that drops under `prefers-reduced-motion`. The 2026-09-19 second pass closed **T-08b** ("banner is bright violet, profile is amber") and **E-01** (the stage-2 visual pass) — his two notes on the screenshot are built: the blank picture areas now say the game draws its own, and a blank display name is nudged like a broken handle. It also turned up one honest limitation: **a mod's accounts are not removed when the mod is uninstalled by deleting it from disk** — the SDK documents it, the game drops the mod's quests but keeps accounts created through `createUser`, and the unload hook can never run for a mod that is gone. Question 11 filed, and the row rewritten as **T-15c** (disable the mod in the game's Mods list while running, which is the path our cleanup *can* serve). Two rows are still open — **T-15c** and the **abandon half of T-11b** — written out step by step in [`reference/sdk-0.24-qa/STATUS.md`](reference/sdk-0.24-qa/STATUS.md) and [the round plan](docs/plans/r185-twotter-return.md) (editor export **1.0.23** beside raw harness **1.0.18**, account `qe24_editor`, `qe24 run tw1` / `tw2` / `tw3` and `qe24 twotter audit`). **Round closed 2026-09-19**: every QA row green or resolved — the abandon half of T-11b last (`twotter: removeUser(qe-tw-account) -> true (the last quest that needs it ended)`, audit `not on this save`), T-15c green (disabling the mod in the Mods list removes the accounts when the game applies it on restart), T-15b red by design (a mod deleted from disk can never run mod code — question 11). Two developer questions came out of the round: §11 and §13. |
...
| # | Item | Notes |
|---|---|---|
| r196 | **The Twotter round closes — every row green** | ([plan](docs/plans/r185-twotter-return.md)) The last two rows landed on 2026-09-19. **T-11b abandon:** `OnAbandon` → `cleanup starting (abandon): 5 item(s) to undo` → `twotter: removeUser(qe-tw-account) -> true (the last quest that needs it ended)` → the tweets swept → audit `not on this save` — the r185 failure (an abandon that kept the account) is closed. **T-15c:** disabling the export in the game's Mods list makes the game unload the package at the next start, and the accounts go with it (`removeUser … -> true (mod unloaded)`), audit `not on this save` afterwards. Also measured: a **plain quit runs no hook** (nothing to clean), and a mod **deleted from disk** can never run mod code — the one leak, filed as question 11. r193's export-loaded readout caught a **game bug** on the way (a disabled mod stays disabled across a new version, a folder deletion and a fresh save — question 13). |
...
Counted from the code at build `2026-09-18.r196`: **1,724 tests** across 86
