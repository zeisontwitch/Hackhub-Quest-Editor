...
| 12 | **Tool packs & Editor Mods** | Community tool mods — exploitation frameworks, recon suites — already expose a pure-data cross-mod contract: targets are live network state, reaction is namespaced events, registration is SharedStorage JSON. r135 shipped the zero-code half — free-form trigger events pinned end-to-end (a quest completes an objective on a third-party tool's event) and the Community tools Cookbook card. Design v2 staged in [`docs/plans/r135-tool-packs-design.md`](docs/plans/r135-tool-packs-design.md): declarative packs (events / storage / target rules / command data), then Zeis's north star — **Editor Mods**: community-authored nodes with their own palette category and colours, declarative emission only. Awaiting go; behind the r131 builds. |
| 10 | **Campaign template + Kisscord contact lifecycle** | r131's read of the Journalist's Sister line (13 hardcoded quests) staged three proposals, **approved by Zeis**: a multi-quest campaign template (chain via `fx.claimQuest`, one world-owner per campaign), exposing the SDK's Kisscord contact lifecycle (`createUser`/`addFriend`/`changeStatus` — the "new contact appears mid-story" move, unmodeled today), and a Campaign cookbook card. See [`docs/plans/r131-journalists-sister-analysis.md`](docs/plans/r131-journalists-sister-analysis.md). The campaign half waits on two in-game checks (world persistence mid-line and line-end teardown) but the contact lifecycle does not. |
...
| Round | Item |
|---|---|
| r134 | **Website polish** ([plan](docs/plans/r134-website-polish.md)): the preview is now a walkable site — an address bar plus internal-link navigation (the sandboxed iframe posts internal link clicks out; the builder serves the linked page, or a friendly not-found) — and **Import folder** turns a folder of AI-written `.html` files into pages in one go (filenames become paths, `<title>`s become titles, existing paths are skipped). |
