...
| # | Item | Notes |
|---|---|---|
| 1 | **Mail cleanup / replyable mail QA** | ([plan](docs/plans/r209-mail-qa-plan.md)) **The probe is built** — raw harness **1.0.24**, `qe24 mail send/audit/remove/watch/cleanup/unload/bounce` plus the `qe24 run mail` quest — and **rows M-01…M-10 wait on one batched session**. The three questions it decides: the reply's promised `repliedTo` field is absent from the declared `Mail.Sent` payload (does it exist at runtime? can a quest match a reply at all?); whether the Reply button draws on the direct `Mail.send({ replyable })` path (M-04) and the quest path the editor ships (M-06); and the collect-ids-remove-on-unload prescription, measured through quest end (M-07) and the disable-then-restart hook (M-08). Authoring is the next round, evidence-gated. |
| 2 | **Date deprecation warning (`moment` RFC2822)** | Attribution corrected — it was never mail. The developer's BUG 10 answer: Twotter/Kisscord dates written with `Date.toString()` in mod content; mail timestamps are engine-set numeric world time; the 30–90 s delay was the mail queue's delivery timeout, a coincidence ([docs/07](docs/07-dev-response-mod-sdk-bug-report-response.md)). Our own measurement agrees: the one warning in the r185 session (game 1.3.1) came from a **game** tweet's `sendedAt`, and our ISO stamps never trigger one (STATUS T-09b). One residual look rides the mail QA batch (**M-09**: clean save, no mods; then the mail rows) — then this row closes or is re-filed as game content. |
| 3 | **SDK 0.24 follow-up choices** | Wi-Fi, phone end-flow / quest-ending APIs, the `Ask player` prompt node, and the Scheduler/Time (as the r172/r173 **Timer** node with delay, relative units and fixed-date modes) are exposed and verified. **S-04 is answered** (clock zone: local), and the QA folder is closed with no pending checks — [`reference/sdk-0.24-qa/STATUS.md`](reference/sdk-0.24-qa/STATUS.md) is the ledger. Remaining queue: the phone-proxy/eavesdrop investigation. The **Twotter re-implementation** is done and closed (see **Done recently**, r196). HTTP/curl/DNS collaborator nodes stay fenced until SteelWaffe answers the upstream gaps. See [`docs/plans/r167-wifi-exposure-and-sdk024-roadmap.md`](docs/plans/r167-wifi-exposure-and-sdk024-roadmap.md). |
...
| r208 | **Pack extras: verified in game, and the page it owed** | Six rows in one session, six greens: the claim entry puts the quest in the journal, the handbook entry opens the handbook (its own landing page — Q15, the game's), **both right-click entries draw** (they had never drawn anything: the old refusals hit them like the menu item), a picture on a menu entry **shows**, and a language the pack has no words for falls back to English rather than showing tokens. In the editor: the picture is now a **file picker** (the field was a text box asking for a picture's written form, which no author could produce), and the handbook grew the section this feature owed — `guides.html#pack-extras`. The batch rule is now written down in the handoff: every open question goes into one session. |
...
Counted from the code at build `2026-09-19.r208`: **1,784 tests** across 88
