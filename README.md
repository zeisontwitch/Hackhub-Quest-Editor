...
| # | Item | Notes |
|---|---|---|
| 1 | **Auto-generate dice — long tail** | The core shipped in r161 (employer, devices, users, every IP field, domains). Remaining fields to fit with a dice: port/vuln Version (service banners) and Service, Pay node From IBAN + From name, Wi-Fi SSID, and the sims (Mail from/to, Kisscord handle, WeeChat host + per-line username). Same pure `lib/generate` engine — just new descriptors/wiring. |
...
Counted from the code at build `2026-09-14.r161`: **1,493 tests** across 77
