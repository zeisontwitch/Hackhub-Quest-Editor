...
| # | Item | Notes |
|---|---|---|
| 1 | **Cheap wins: pack extras and localization** | ([plan](docs/plans/r199-cheap-wins-plan.md)) **Stage A ran (r200) and three of four came back green:** desktop widgets render a mod's own HTML (with the wrinkle that `transparent` defaults to true — a widget without a background is the default, so the editor will default it to opaque), right-click entries work on files, and localization works with **30 languages** to offer. The start-menu item did not appear. **r201 re-probes it** with all three section spellings plus the transparency A/B — rows **T-20..T-22** in [`STATUS.md`](reference/sdk-0.24-qa/STATUS.md). Stage B (the Pack extras dialog + `{{tr.…}}` localization) starts once those readings are in. |
