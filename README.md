...
| # | Item | Notes |
|---|---|---|
| 1 | **Cheap wins: pack extras and localization — first run done, second run A…F open** | ([plan](docs/plans/r199-cheap-wins-plan.md)) **Stage A closed: all four APIs work in game.** **Stage B is built.** The first run of the rows (export 1.0.28) came back **two green** — the widget drew its own background, both right-click entries appeared — with three findings: the start-menu entry was there but a click did nothing visible, the labels did not translate after a language switch, and the widget stayed English (by design; its file cannot be translated). Two of those are answered in the build: **every click now logs a line before it does anything** (a dead click and a message API that draws nothing look identical on screen), and **labels are handed over again when the language changes** via the SDK's `onLanguageChange`. The third is a project-wide question now: **`UI.notify` has never been seen to draw anything in 1.3.1** — every notification QA ever saw was a toast, while the editor's own Notify node uses `notify` by default. Rows **A…F** in `reference/sdk-0.24-qa/editor-export/README.md` settle it with two looks and one log paste. Install export **1.0.29** + harness **1.0.22**. |
