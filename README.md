...
| # | Item | Notes |
|---|---|---|
| 1 | **Cheap wins: pack extras and localization — built, five rows to run** | ([plan](docs/plans/r199-cheap-wins-plan.md)) **Stage A closed: all four APIs work in game.** **Stage B is built (r203)**: a Pack extras dialog (start menu entries, desktop widgets, right-click entries) plus a translation table of the game's own 30 languages, with `{{tr.…}}` resolving in every text field. Two readings shaped it — no `section` control (the game ignores it) and widgets default to opaque (the SDK default is see-through) — and one trap is handled: a quest's Title and the menu labels are read at *registration*, so the runtime translates those before handing them over, leaving `{{data.…}}` alone everywhere else. **Open: rows T-23…T-27 in game**, run against export **1.0.28** — rows in `reference/sdk-0.24-qa/editor-export/README.md`, and copy the whole `mod/` folder (its `widgets/` folder is the widget). A manual page for the feature is owed once the rows are green. |
...
Counted from the code at build `2026-09-19.r203`: **1,760 tests** across 88
