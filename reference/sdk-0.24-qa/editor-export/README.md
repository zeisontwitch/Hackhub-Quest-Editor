...
|---|---|---|
| 1.0.51 | 2026-09-21.r227 | **No functional change to compiled quests** — the round is editor polish from Zeis's r226 review: the comment dice aligns with its field, and the author-facing copy is plain language (no code talk; the Anonymous toggle's explanation moved into a mouse-over ⓘ). |
...
| 1.0.44 | 2026-09-21.r220 | **No functional change** — the build stamp. The round went into the QA harness (1.0.28: the editor-clone grid HF-6…HF-10) and a new canary mod (`qe24-feedcanary`, editor-identical manifest). Editor blurbs gained the in-game avatar finding (name without avatar draws a broken icon). |
| 1.0.43 | 2026-09-21.r219 | **No functional change** — the build stamp inside the packed mod. The round went into the QA harness instead (1.0.27: the `qe24 feed` probe grid, five post variants). Reinstalling this export changes nothing for the feed investigation. |
| 1.0.42 | 2026-09-21.r218 | **No runtime change — the bare-post guidance goes on record.** The v3 retest failed flawless (fresh name+id, named commenter, extracted assets, contract-clean shapes), so r218 files docs/03 §21 (mod quest posts stopped surfacing on 1.3.1; the held `Queue.HandleQuestHackhubPosts` job) and the compiler's feed-post notice now carries the field evidence: the only shape we have ever seen render is a bare post. Stamp changed. |
