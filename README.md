...
| 2 | **Twotter (probe running)** | The raw harness now ships `qe24 twotter` — rows T-01…T-07 in [`reference/sdk-0.24-qa/STATUS.md`](reference/sdk-0.24-qa/STATUS.md), the one open probe. It plants the r31 bad-record shape on purpose, checks whether 1.3.0's repair-on-load works, exercises `updateUser`/`removeUser`/`postTweet`, and checks the quest-declared write path. Re-implementation follows the results. History: the r31 removal was forced by a game crash (a quest-declared account with an undefined `bio`, which Twotter's search called `.toLowerCase()` on), and the 1.3.0 changelog fixes exactly that — "a content pack could break Twotter permanently, taking the site down when searching for people. Affected saves are repaired on load" — plus 1.3.1's Twotter UI fixes. Next round is the feasibility read: what the 0.24 declarations offer (`Twotter.updateUser`, `Twotter.removeUser`, account/post APIs), what the crash actually required, and whether the fences can come down. Then the update/remove QA.ify in game that they repair/remove save records, posts, follows and search safely before restoring Twotter authoring. |
