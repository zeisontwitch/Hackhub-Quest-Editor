...
| 12 | **Tool packs — Editor Mods** | r137 shipped the first rail: a pack is one `toolpack.json` of pure data ([`docs/ToolPack-Format.md`](docs/ToolPack-Format.md), starter pack in `reference/example-toolpack/`). Loaded packs join the trigger picker as a Community group and their SharedStorage data shapes become "Community data" nodes (labelled inputs, entry template snapshotted into the project, honesty warning that the player needs the game mod). Still open: **Editor Mods** — pack-authored nodes in the palette (the `nodes[]` section is reserved), declarative emitters, pack-driven target-rule surfaces. Awaiting Zeis's go on those. |
