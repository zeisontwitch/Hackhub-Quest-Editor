...
| # | Item | Notes |
|---|---|---|
| 1 | **Mail cleanup / replyable mail QA** | ([plan](docs/plans/r209-mail-qa-plan.md)) SDK 0.24 added `Mail.remove(id): boolean` and `replyable` on `MailDefinition`, but three things need in-game evidence before any authoring: the reply's promised `repliedTo` field is absent from the declared `Mail.Sent` payload; the editor's replyable path still assumes `MailDefinition` has no reply flag (stale); and the collect-ids-remove-on-unload prescription is unmeasured. Rows **M-01…M-09**, one session, raw harness **1.0.24**. Authoring (cleanup surface, replyable-path simplification, docs/03 question if replies can't be matched) is the next round, only for what the rows prove. |
