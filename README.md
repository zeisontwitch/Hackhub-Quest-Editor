...
| # | Item | Notes |
|---|---|---|
| 1 | **Cheap wins: pack extras and localization — three rows open (I, J, K)** | ([plan](docs/plans/r199-cheap-wins-plan.md)) **Stage A closed**, **Stage B built and working**. In game: the widget, both right-click entries, the German labels and the German quest title are green; **`UI.notify` and `UI.toast` both draw**. The click on a pack's menu item reached the pack and was refused for lack of a mod identity (`[ContentSDK] Mod "null" … without "ui" permission`) — and the r205 probe proved that a one-millisecond `Scheduler` job **gets the identity back**, so **r206 hands every click action to the engine** and runs it in the callback: `Quest.claim` and `Mail.send` from a click were refused, and the same calls from the callback are what rows I…K now test. Also learned there: a click cannot see the mod's translation table either (it logged the raw key), and the callback gets that back too. **Q14** stays open for the developers. Install export **1.0.31**; harness unchanged at **1.0.23**. |
...
Counted from the code at build `2026-09-19.r206`: **1,774 tests** across 88
