...
| # | Item | Notes |
|---|---|---|
| 1 | **Cheap wins: pack extras and localization — six rows in one batch (L…Q)** | ([plan](docs/plans/r199-cheap-wins-plan.md)) **Stage A closed**, **Stage B built and working in game**. Green so far: the widget, both right-click entries, German labels and quest title, the message action (a German sentence from the engine's callback, not the raw token a click logs) and the mail action (a letter arrived). The click refusal was a missing mod identity, and **r206 hands every click action to the engine, which fixes it** — the log proves the round trip. **r207 fixes the claim action**, which sent the editor's own id where `Quest.claim` wants the game's name for the quest (the game is silent about an id it does not know, so the click looked dead). **Q15**: `Handbook.open` opens the handbook at its own landing page rather than the page asked for — the article ids are not published, so the editor no longer claims the page is reached. One batched run is open (export **1.0.33**, harness unchanged at **1.0.23**): the claim fix, the handbook's honest log line, the two right-click actions (never drawn), the `icon` field and the language fallback. |
