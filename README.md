...
| 12 | **Tool packs — a mod for our mod** | Community tool-mod authors drop a declarative `toolpack.json` folder into the editor and their tools load as palette nodes: fields become inspector fields, answers emit through the `world.toolResponse` rail (`addCommandData`), and the editor warns that the quest requires that tool mod on the player's machine. Design staged in [`docs/plans/r135-tool-packs-design.md`](docs/plans/r135-tool-packs-design.md) — awaiting go; one QA item gates it (does `addCommandData` surface for mod-registered commands in-game?). Behind website polish and the r131 builds. |
