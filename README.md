...
| # | Item | Notes |
|---|---|---|
| 1 | **Cheap wins: pack extras and localization — plan for review** | ([plan](docs/plans/r199-cheap-wins-plan.md)) Zeis picked the two cheap wins from the [coverage audit](docs/plans/r198-sdk-coverage-audit.md). **Plan only — nothing built yet.** Stage A is a harness probe (`qe24 extras on/off/lang`, one loud widget HTML) that answers whether 1.3.1 honours `Menu.addItem` (and its top/bottom section), `Desktop.addWidget` from a mod-root HTML path, `ContextMenu.register` per target, and whether `Localization.t()` resolves in game — plus the real language list and which permission each wants; four QA rows come out of it. Stage B is a **Pack extras** dialog (start-menu items, desktop widgets, right-click items) whose widget HTML reuses the website builder's page editor, plus **localization** as a Languages table emitted through `Localization.register` and a `{{tr.…}}` token that every text field in the editor inherits from the existing `{{player.ip}}`-style filler — no per-field UI. The one open decision: what a click *does* (recommended: notify / start a quest / send mail / open a handbook page, as a dropdown). |
