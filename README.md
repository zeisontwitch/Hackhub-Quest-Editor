...
| # | Item | Notes |
|---|---|---|
| 1 | **Finish a full run of either template** | Harbour reaches the scan; the Ledger reaches the exploit. r71 removed the destroy-before-create race that was deleting the network the mod had just built, so a re-export should now scan cleanly. |
...
| # | Item | Notes |
|---|---|---|
| 5 | Right-Click node search | On canvas with auto-complete suggestion |
| 6 | Properly styled color-wheel | With preset colors and RGB/HSL sliders; used when colouring groups (rather than the OS-specific color wheel) or anything else that might be coloured in the future |
| 7 | Wire/noodle physics | Pulling a wire makes it hang and bounce; past a distance it pulls straight; releasing snaps it back before it disappears. Sag derived from slack, ~200ms non-interactive ghost on delete, physics only on the held wire, must honour the wire-motion toggle. Ensure this doesn't tank the render performance or cause lags. **Must write inside the canvas, never the document root** (see r42). |
| 8 | "Contact-driven story" template | Phone brief → Kisscord drip gated on objectives → WeeChat timed to a beat. |
| 9 | "Branching consequence" template | A choice that changes which ending the player gets. |
...
| Round | Item |
|---|---|
| r99 | **Align was never getting the cards' sizes** (plan in `docs/r99-alignment-plan.md`). A probe on the mounted canvas logged what the click handler actually passes to `alignPositions`: `size: undefined` for every node. Sizes came from our own `measured` map, filled from React Flow "dimensions" events; a missing size counts as zero, so "align centres" silently degraded to "align top-left corners" — the cards' middles stayed staggered and the wires between them still curved, which is exactly the reported picture. r97 and r98 were therefore never exercised on real cards. Sizes now come from React Flow's `nodeLookup`, which its own ResizeObserver keeps authoritative. New `alignMeasured.test.tsx` drives the mounted canvas with real geometry — the case every previous test missed. |
| r98 | **Alignment rebuilt against how design tools actually behave** (plan and evidence in `docs/r98-alignment-plan.md`). The real defect: with Snapping on, `arrange()` centred the cards and then snapped each **corner** to the grid, which pushed cards of differing heights back off the shared line — 8px apart for the reported 62px/78px pair, so r97's fix silently undid itself. The grid is now applied to the shared **line**, once. Two correctness changes from the Photoshop docs: alignment targets the **bounding-box centre of the selection** (what "Align Vertical Centers" means) rather than the mean of the individual centres, and **Even** now equalises the *gaps* between cards using their measured sizes, instead of spacing their corners — with cards of different widths those are not the same thing, and equal gaps is what the word implies. |
| r97 | **Alignment now centres the cards, and the toolbar says what it does.** Aligning matched top-left corners, so a short "Tool response" and a tall "Dialogue" ended up with staggered middles — visibly "barely moved" even though it had worked. It now uses React Flow's measured sizes to line up centres, falling back to corner alignment when a node has not been measured yet. The arrange buttons were also icon-only, which made it easy to press "Even across" (a no-op on already-evenly-spaced nodes) while meaning "Row"; every button now carries a word: **Row · Column · Even across · Even down**. Wire toggle relabelled **Animated wires / Static wires**, with tooltips that start "Editor only:" so it cannot read as a game setting. |
| r96 | **Snap to grid, align and spread** (roadmap 5 and 6). Four new toolbar buttons line the selected nodes into a row or column, or even out their gaps; a Free/Snapping toggle turns on grid snapping while dragging. Alignment uses the *average* of the selected positions, so a group stays where the author put it instead of jumping to whichever node happens to be first. All the arithmetic is pure (`arrange.ts`) and applied in **one** `arrangeNodes` write — a new store action, because `setNodePositions` deliberately records no history for live dragging, which would have made an alignment impossible to undo (caught by a test). The snap preference is a per-author setting in localStorage, not part of the project. Also added **standing performance guards** (`canvasPerformance.test.tsx`) that fail the build on the faults this canvas has shipped before: an idle rAF loop, a custom property on the document root, and canvas re-renders from unrelated store writes. Verified by reintroducing r38's fault and watching them fail. |
| r95 | **Ctrl+drag over already-selected nodes.** r94 put the box arithmetic in `onNodesChange`, which never runs for this gesture: `getSelectionChanges` only emits a change when a node's selected state *differs* from what the box wants, and ctrl+dragging over a live selection means `true !== true` → nothing sent. Reproduced it first — the probe logged zero callbacks. The box result is now computed at `onSelectionEnd` from React Flow's own `nodeLookup` geometry (`nodesInBox`), so it no longer depends on a change event existing. The box rect is captured via a store subscription rather than our pointer listeners, which run in the capture phase and would read the pre-update value — at pointerup, the zero-size rect from pointerdown. Both ctrl+drag gestures now have real end-to-end tests. |
| r94 | **Shift+drag and Ctrl+drag** (plan and evidence in `docs/r94-selection-gesture-plan.md`). Two findings from instrumenting the real handler rather than reading the source. (a) The drag-start `resetSelectedElements()` fires one line *before* `onSelectionStart`, so our `boxSelecting` flag was still false when the clear arrived and the guard never matched — with a modifier held that clear is now captured and used as the drag's starting selection. (b) **Ctrl+drag to deselect does not exist in React Flow at all**: `commitUserSelectionRect` only ever emits `selected: true` for nodes inside the box. Both add and subtract are now computed by us in `boxSelectionResult`. Also replaced the wrapper-level modifier listener with one on `window`: React Flow calls `setPointerCapture` on pointerdown, which retargets later pointer events away from our wrapper, so the flag went stale mid-drag — invisible in jsdom, which implements neither `PointerEvent` nor `setPointerCapture`. |
| r93 | **Found the actual cause of the ctrl+click failure: React Flow's `nodesselection` overlay.** When more than one node is selected it renders a div across their bounding box so the group can be dragged as one — sitting *on top* of the nodes. Every click landed on the overlay, so ctrl+click never reached a node to deselect it, and that overlay was the "selection box" that appeared stuck. It is now hidden with `pointer-events: none`; we already move groups ourselves, so it was pure cost. Modifier state is also read from the real pointer/keyboard event in the capture phase instead of React Flow's `multiSelectionActive`, which misses the key when it goes down before the canvas has focus. Pinned by `selectionGestures.test.tsx`, which drives the mounted canvas with real events — the pure-function tests passed through two rounds of this bug being live. |
| r92 | **Corrected r91, which did not work.** Two of my own mistakes. (a) The additive rule dropped *every* clear-only batch while a modifier was held — but ctrl+click-to-deselect is a clear-only batch too, so it swallowed exactly the thing it was meant to enable. A test asserted the broken behaviour (`toBeNull()`) with a comment rationalising it, so the suite stayed green. The rule now applies only while a selection box is actually open, which is what distinguishes drag tidy-up from a deliberate deselect. (b) The `onPaneContextMenu` handler added for the stuck rectangle was dead code: React Flow's pane returns early and never calls it when `panOnDrag` includes button 2. Right-drag panning removed, so the handler runs and the rectangle clears. |
| r91 | **Modifier-held selection** (follow-up to r90). Shift+box-drag deselected everything first: React Flow calls `resetSelectedElements()` at the start of every box drag with no modifier check, so the wholesale clear arrived before the new box's picks. A clear-only batch is now ignored while a multi-select modifier is held, read from React Flow's own `multiSelectionActive` at the moment the change arrives rather than tracked with our own key listeners (which drift when the window loses focus mid-drag). Ctrl+click also left a translucent selection box stuck on the canvas: on Windows and Linux Ctrl+click raises `contextmenu` before any pointerup reaches the pane, so React Flow never cleared its rectangle — the pane now swallows that context menu. |
...
| r70 | Removed the Hackertyper node: it built a bespoke web page rather than using any game feature, and the real scripted-reply mechanic (`isMine`) already lives in the dialogue editor. Removing it exposed a live bug — a typed answer resumed the flow down a socket (`out`) that node does not have, so everything wired to **Correct** was dead. Three templates also shipped an "On start & reload" node wired to nothing. |
| r69 | The game's real scripted-reply mechanic is `isMine: true` on a chat message, which the dialogue editor already offers — the Hackertyper node is a hand-built web page, not that mechanic, and is renamed **Typed reply page** to stop implying otherwise. Two bugs found on the way: WeeChat player lines were sent as an NPC called "you" (no `isMine`), and the node's "desktop app" / "phone app" surfaces compiled to nothing without warning. |
| r68 | Corrected r66: "The player can reply" is **not** dead. r37's conclusion that `Quest.sendMail` fails came from a session where the mod was not loading at all (r38 found that the next round), and r41 showed the fallback delivering mail successfully. A replyable mail now routes through `Quest.sendMail`, the only shape carrying a reply flag, and falls back to `Mail.send` with a clear log line if the engine has no copy. |
