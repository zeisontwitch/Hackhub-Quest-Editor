...
| r220 | **The grid rendered — shapes innocent, two suspects left** | All five harness posts drew in one feed: the file-avatar contract renders (the violet square), name-without-avatar draws a broken icon, and the post fields are innocent — the suppression is mod-shaped. r220 isolates the editor's ALWAYS-ASSIGNED quest fields (HF-6…HF-10) from the manifest shape (a canary mod with editor-identical permissions). |
