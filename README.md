...
| # | Item | Notes |
|---|---|---|
| 1 | **Cheap wins: pack extras and localization** | ([plan](docs/plans/r199-cheap-wins-plan.md)) **Stage A built (r200) and waiting on one run** — all four click actions approved. The harness gained `qe24 extras on / off / lang`: it registers a start-menu item (bottom section), a loud magenta desktop widget whose HTML ships at `mod/widgets/qe24-widget.html`, two right-click items (on a file and on the desktop) and `en`/`de` language bundles, then prints what the game reports it has. Five new tests drive the real harness against a stub SDK; six falsifications went RED. **Rows T-16..T-19 in [`STATUS.md`](reference/sdk-0.24-qa/STATUS.md)** say what to look at and what to paste back. Stage B — the Pack extras dialog and localization with a `{{tr.…}}` token — starts once those readings are in. |
