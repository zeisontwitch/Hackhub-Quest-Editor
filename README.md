...
| r214 | **Mail authoring: playtest green — the round is closed** | W-01…W-04 all green on a fresh save (results: [`reference/sdk-0.24-qa/QE24-TestResults-MailAuthoring.md`](reference/sdk-0.24-qa/QE24-TestResults-MailAuthoring.md)): the Reply button path, the `to` = From reply recipe, withdraw-at-complete (`Mail.remove(MaGUHssZcU …) -> true`) and persistence after a reload. W-05 (abandon) closed on prior evidence. Wrinkle recorded: the probe's Hackhub feed post never surfaced — watch the next feed-post quest. Nothing open in the QA folder. |
