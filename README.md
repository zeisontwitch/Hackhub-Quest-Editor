...
| # | Item | Notes |
|---|---|---|
| 1 | **Cheap wins: pack extras and localization** | ([plan](docs/plans/r199-cheap-wins-plan.md)) **Stage A is closed: all four APIs work in game.** Start-menu items appear in the bottom strip of the start menu (the declared `section` has no visible effect, so the editor won't offer it), desktop widgets render a mod's own HTML with position and size honoured (**and `transparent` defaults to true** — the switch will be visible and defaulted to opaque), right-click items work on files and on the desktop, and localization translates, substitutes placeholders and lists **30 languages**. **Stage B is being built now** — the Pack extras dialog plus localization with a `{{tr.…}}` token, with the four approved click actions (notify, claim a quest, send mail, open a Handbook page). |
