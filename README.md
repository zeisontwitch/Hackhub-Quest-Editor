...
| # | Item | Notes |
|---|---|---|
| 1 | **Which SDK capability to build next — the coverage audit** | ([audit](docs/plans/r198-sdk-coverage-audit.md)) Every SDK 0.24 namespace matched against what the editor can actually emit. Exposed and verified: events, networks, databases, mail/Kisscord/WeeChat, Twotter, shell, handbook, bank transfers, prompts, random, timers, file placement, pack data, websites. **Not exposed at all:** **phone apps** (`PhoneApp`/`RegisterPhoneApp` — own HTML screen, icon and title on the player's phone), **menu entries / desktop widgets / right-click items**, **localization**, **themes**, **guided tours**, **per-pack settings**, **dynamic HTTP endpoints**, **HTTP interception**, **HTTP history/cookies**, **reading a bank balance**, **file delete/move/rename**, and a few Kisscord/Twotter extras. The audit recommends phone apps first (largest missing capability, and the editor's website HTML pipeline is most of the machinery — it needs a harness probe first) with menu/desktop items and localization as the cheap wins; mod-internal APIs (`Variables`, `Time.scale`, `Scheduler.list`) are recommended as declared non-goals. **Zeis's pick decides what gets built.** |
