...
| 2 | **Twotter re-implementation (probe green, plan next)** | The r179 probe is **answered** (r180): a record with `bio: undefined` — the exact shape that crashed the game in r31 and forced the feature out — is searched and opened safely on 1.3.0 / build 25388883, `removeUser` deletes accounts including quest-declared ones, and posts work. So the fence can come down, with three constraints the results impose: author accounts through `createUser`/`addUser` (never trust the declarative path to fill a record), never rely on repair-on-load (it did not repair anything), and hang no objective on `Twotter.AccountCreated` (it does not fire for API accounts). Next: a plan for the authoring round — account + tweet authoring, a post node, and cleanup on quest end — for Zeis to review before it is built. Results: [`STATUS.md`](reference/sdk-0.24-qa/STATUS.md). |
